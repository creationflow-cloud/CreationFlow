...
describe("classifyShortcut", () => {
  ...
  it("returns redo on Ctrl+Y and Ctrl+Shift+Z", () => {
    expect(classifyShortcut({ key: "y", ctrlKey: true }, notInput, emptySelection).action).toBe(
      "onRedo",
    );
    expect(
      classifyShortcut({ key: "z", ctrlKey: true, shiftKey: true }, notInput, emptySelection)
        .action,
    ).toBe("onRedo");
  });

  it("returns save on Ctrl+S", () => {
    expect(classifyShortcut({ key: "s", ctrlKey: true }, notInput, emptySelection).action).toBe(
      "onSave",
    );
  });

  it("returns duplicate on Ctrl+D", () => {
    expect(classifyShortcut({ key: "d", ctrlKey: true }, notInput, oneSelected).action).toBe(
      "onDuplicate",
    );
  });
  ...
  it("nudges by 1px on arrow keys", () => {
    expect(classifyShortcut({ key: "ArrowRight" }, notInput, oneSelected).nudge).toEqual({
      dx: 1,
      dy: 0,
    });
    expect(classifyShortcut({ key: "ArrowDown" }, notInput, oneSelected).nudge).toEqual({
      dx: 0,
      dy: 1,
    });
    expect(classifyShortcut({ key: "ArrowLeft" }, notInput, oneSelected).nudge).toEqual({
      dx: -1,
      dy: 0,
    });
    expect(classifyShortcut({ key: "ArrowUp" }, notInput, oneSelected).nudge).toEqual({
      dx: 0,
      dy: -1,
    });
  });
  ...
  it("nudges by 10px on arrow keys with shift", () => {
    expect(
      classifyShortcut({ key: "ArrowRight", shiftKey: true }, notInput, oneSelected).nudge,
    ).toEqual({ dx: 10, dy: 0 });
  });
  ...
  it("ignores all shortcuts when target is an input", () => {
    ...
    expect(
      classifyShortcut({ key: "a", ctrlKey: true }, inputTarget, emptySelection).action,
    ).toBeNull();
  ...
  it("handles uppercase letters in ctrl shortcuts", () => {
    expect(classifyShortcut({ key: "A", ctrlKey: true }, notInput, emptySelection).action).toBe(
      "onSelectAll",
    );
  });
  ...
  it("triggers group on Ctrl+G and ungroup on Ctrl+Shift+G", () => {
    ...
    const ungroup = classifyShortcut(
