...
export function createRenderWorker(options: RenderWorkerOptions = {}): Worker<RenderJobQueuePayload> {
  ...
  return new Worker<RenderJobQueuePayload>(
    ...
    },
    {
      ...
      settings: {
        backoffStrategy: (attemptsMade: number) => backoffMs * Math.pow(2, Math.max(0, attemptsMade - 1)),
      },
    },
  );
}
